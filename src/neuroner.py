...
INT_PARAMETERS = ['character_embedding_dimension', 'character_lstm_hidden_state_dimension',
...
FLOAT_PARAMETERS = ['dropout_rate', 'learning_rate', 'gradient_clipping_value']
BOOL_PARAMETERS = ['remap_unknown_tokens_to_unk', 'use_character_lstm', 'use_crf', 'train_model',
...
class NeuroNER(object):
    ...
    def _create_stats_graph_folder(parameters):
        ...
        :param parameters: used to specify the path to text and output folder
        :return: 
        """
    ...
    def _check_parameter_compatiblity(parameters, dataset_filepaths):
        ...
        if parameters['train_model']:
            if 'train' not in dataset_filepaths or 'valid' not in dataset_filepaths:
                raise IOError("If train_model is set to True, both train and valid set \
                must exist in the specified dataset folder: {0}".format(parameters['dataset_text_folder']))
    ...
    def _load_parameters(self, parameters_filepath, arguments=None, verbose=True):
        """
        ...
        if len(parameters_filepath) > 0:
            ...
            for k, v in nested_parameters.items():
                parameters.update(v)
        # Ensure that any arguments specified in the command line overwrite parameters specified in the parameter file
        if arguments:
        ...
        for k, v in parameters.items():
            ...
            if k in INT_PARAMETERS:
                parameters[k] = int(v)
            elif k in FLOAT_PARAMETERS:
                parameters[k] = float(v)
            elif k in BOOL_PARAMETERS:
        ...
        if parameters['use_pretrained_model']:
            pretraining_parameters = self._load_parameters(
                ...
                verbose=False)[0]
            warning_params = ['use_character_lstm', 'character_embedding_dimension',
                              'character_lstm_hidden_state_dimension', 'token_embedding_dimension',
                              'token_lstm_hidden_state_dimension', 'use_crf']
            for name in warning_params:
                if parameters[name] != pretraining_parameters[name]:
                    print('WARNING: parameter {0} was overwritten from {1} to {2} to\
                     be consistent with the pretrained model'.format(name, parameters[name],
    ...
    def __init__(self,
        ...
        session_conf = tf.ConfigProto(
            ...
            # automatically choose an existing and supported device in case the specified one doesn't exist
        ...
        self.sess = sess
        if parameters['train_model']:
            self.stats_graph_folder, self.experiment_timestamp = self._create_stats_graph_folder(parameters)
        else:
    ...
    def fit(self):
        ...
        results = {'epoch': {},
                   'execution_details': {'train_start': start_time,
                                         'time_stamp': self.experiment_timestamp,
                                         'early_stop': False,
        ...
        model_folder = os.path.join(self.stats_graph_folder, 'model')
        utils.create_folder_if_not_exists(model_folder)
        with open(os.path.join(model_folder, 'parameters.ini'), 'w') as parameters_file:
            conf_parameters.write(parameters_file)
        pickle.dump(dataset, open(os.path.join(model_folder, 'dataset.pickle'), 'wb'))
            
        tensorboard_log_folder = os.path.join(self.stats_graph_folder, 'tensorboard_logs')
        utils.create_folder_if_not_exists(tensorboard_log_folder)
        tensorboard_log_folders = {}
        for dataset_type in dataset_filepaths.keys():
            tensorboard_log_folders[dataset_type] = os.path.join(self.stats_graph_folder, 'tensorboard_logs',
                                                                 dataset_type)
        ...
        try:
            while True:
                ...
                y_pred, y_true, output_filepaths = train.predict_labels(sess, model, transition_params_trained,
                                                                        ...
                                                                        self.stats_graph_folder, dataset_filepaths)

                # Evaluate model: save and plot results
                evaluate.evaluate_model(results, dataset, y_pred, y_true, self.stats_graph_folder, epoch_number,
                                        epoch_start_time, output_filepaths, parameters)

                if parameters['use_pretrained_model'] and not parameters['train_model']:
                    conll_to_brat.output_brat(output_filepaths, dataset_brat_folders, self.stats_graph_folder)
                    break
        ...
        evaluate.save_results(results, self.stats_graph_folder)
        for dataset_type in dataset_filepaths.keys():
    ...
    def predict(self, text):
        ...
        tokenized_text = brat_to_conll.text_to_conll(text, self.parameters['tokenizer'],
