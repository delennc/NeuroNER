...
class EntityLSTM(object):
    ...
    def __init__(self, dataset, parameters):
        ...
        if parameters['use_character_lstm']:
            ...
            # Idea: reshape so that we have a tensor [number_of_token, max_token_length, token_embeddings_size],
            # which we pass to the LSTM
        ...
        if parameters['use_crf']:
            with tf.variable_scope("crf") as vs:
                ...
                # Compute the log-likelihood of the gold sequences and keep the transition params
                # for inference at test time.
        ...
        for token in dataset.token_to_index.keys():
            if token in token_to_vector.keys():
                ...
                number_of_token_lowercase_found += 1
                    ...
                    and re.sub('\d', '0', token) in token_to_vector.keys():
    ...
    def restore_from_pretrained_model(self, parameters, dataset, sess, token_to_vector=None):
        pretraining_dataset = pickle.load(open(os.path.join(parameters['pretrained_model_folder'], 'dataset.pickle'),
                                               'rb'))
        ...
        if pretraining_dataset.index_to_token == dataset.index_to_token \
        ...
        # If the token and character mappings are different between the pretrained model and the current model
            ...
            # Resize the token and character embedding weights to match them
            # with the pretrained model (required in order to restore the pretrained model)
